...
final class LayoutEngine {
    ...
    init(spreadsheetView: SpreadsheetView, scrollView: ScrollView) {
        ...
        // LEARNING-SCROLL 8/8：每个区域使用“自己的 offset + 自己的 frame”生成可见矩形。
        // 因为第 5 步只同步指定轴：tableView 双轴变化，rowHeaderView 只变 x，
        // columnHeaderView 只变 y，cornerView 两轴不变，于是四块区域自然呈现不同滚动行为。
    ...
    /// LEARNING: 单个内部 ScrollView 的一次可见区域布局：
    /// 二分查找起始行 -> 枚举可见行列 -> 渲染 Cell/线条/边框 -> 回收不可见对象。
    func layout() {
    ...
    /// LEARNING: Cell 生命周期的分岔点。
    /// 已可见：只更新 frame；新进入可见区：询问 dataSource、写入状态并添加到视图。
    ...
    /// LEARNING: 当前布局结束后，用“旧可见地址 - 新可见地址”找到离屏对象。
    /// Cell 被 removeFromSuperview 后按 reuseIdentifier 回到复用池，下次 dequeue 时复用。
